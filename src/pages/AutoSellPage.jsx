...
import { Zap, Trash2, TrendingUp, RefreshCw, ChevronDown, ChevronUp, History, Clock, Loader2 } from "lucide-react";
...
export default function AutoSellPage() {
    ...
    const {
        ...
        queryKey: ['autoSellRules', 'active', currentPage],
        queryFn: async () => {
            const res = await axiosInstance.get(API_ENDPOINTS.AUTOSELL.PAGED, {
                params: {
                    page: currentPage,
                    size: PAGE_SIZE,
                    sortBy: "createdAt",
    ...
    const {
        ...
        queryKey: ['autoSellRules', 'history', historyPage],
        queryFn: async () => {
            const res = await axiosInstance.get(API_ENDPOINTS.AUTOSELL.PAGED, {
                params: {
                    page: historyPage,
    ...
    const deleteMutation = useMutation({
        mutationFn: (id) => axiosInstance.delete(API_ENDPOINTS.AUTOSELL.BY_ID(id)),
        onSuccess: () => {
            queryClient.invalidateQueries(['autoSellRules', 'active']);
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                        Manage your automated selling triggers.
                    </p>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    {rules.length === 0 ? (
                        <div className="text-center py-12 glass-card">
                            ...
                            <h3 className="text-lg font-medium text-foreground">No active rules</h3>
                            <p className="text-muted-foreground mb-4 font-normal">You haven't set any auto-sell rules yet.</p>
                        ...
                        <div className="rounded-xl border border-border bg-card overflow-hidden">
                            <div className="px-4 py-3 bg-secondary/30 border-b border-border">
                                <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
                                    <Zap className="h-4 w-4 text-orange-400" /> Active rules
                                </h3>
                            </div>
                                    ...
                                    <TableBody>
                                        {rules.map(rule => (
                                            <TableRow
                                                ...
                                                onClick={() => navigate(`/stock/${rule.stockId}`)}
                                            >
                                                <TableCell>
                                                    <div>
                                                        <p className="font-semibold text-foreground">{rule.symbol}</p>
                                                        <p className="text-xs text-muted-foreground">{rule.stockName}</p>
                                                    </div>
                                                </TableCell>
                                                <TableCell className="text-right">
                                                    {rule.targetPrice ? (
                                                        <span className="font-mono font-medium text-green-500">≥ ₹{rule.targetPrice.toLocaleString()}</span>
                                                    ) : (
                                                ...
                                                <TableCell className="text-right">
                                                    {rule.stopLoss ? (
                                                        <span className="font-mono font-medium text-red-500">≤ ₹{rule.stopLoss.toLocaleString()}</span>
                                                    ) : (
                                                        <span className="text-muted-foreground">—</span>
                                                    )}
                                                </TableCell>
                                                <TableCell>
                                                    <Badge variant="outline" className="bg-orange-500/10 text-orange-400 border-orange-500/30 text-xs text-nowrap">
                                                        Active
                                                    </Badge>
                                                </TableCell>
                    ...
                    <div className="rounded-xl border border-border bg-card overflow-hidden">
                        ...
                        {showHistory && (
                            <div className="border-t border-border animate-fade-in-down">
                                {(loadingHistory || (isFetchingHistory && historyRules.length === 0)) ? (
                                        ...
                                        <Table>
                                            ...
                                            <TableBody>
                                                {historyRules.map(rule => (
                                                    ...
                                                    >
                                                        <TableCell>
                                                            <div className="italic text-foreground font-semibold">
                                                                {rule.symbol}
                                                            </div>
                                                        </TableCell>
                                                        <TableCell className="text-right font-mono text-xs">
                                                            {rule.targetPrice && <span className="text-green-500/80">TP: ₹{rule.targetPrice}</span>}
                                                            {rule.targetPrice && rule.stopLoss && <span className="mx-1 text-muted-foreground">|</span>}
                                                            {rule.stopLoss && <span className="text-red-500/80">SL: ₹{rule.stopLoss}</span>}
                                                        </TableCell>
                                                        <TableCell>
                                                        ...
                                                        <TableCell className="text-right text-xs text-muted-foreground">
                                                            {new Date(rule.triggeredAt).toLocaleString()}
                                                        </TableCell>
                                                    </TableRow>
                                                ))}
                                            </TableBody>
